 ...
 * <p>
 * This implementation encapsulates all steps which are necessary to create a fully usable and populated device metadata graph of a particular UPnP device. It
 * starts with an unhydrated and typically just discovered {@link org.teleal.cling.model.meta.RemoteDevice}, the only property that has to be available is its
 * {@link org.teleal.cling.model.meta.RemoteDeviceIdentity}.
 * </p>
 * <p>
 * This protocol implementation will then retrieve the device's XML descriptor, parse it, and retrieve and parse all service descriptors until all device and
 * service metadata has been retrieved. The fully hydrated device is then added to the {@link org.teleal.cling.registry.Registry}.
 * </p>
 * <p>
...
public class RetrieveRemoteDescriptors implements Runnable {
	...
	protected void describe() {
		...
		if (deviceDescMsg.getOperation().isFailed()) {
			log.warning("Device descriptor retrieval failed: " + rd.getIdentity().getDescriptorURL() + ", " + deviceDescMsg.getOperation().getResponseDetails());
			return;
		}
	...
	protected RemoteDevice describeServices(RemoteDevice currentDevice) throws DescriptorBindingException, ValidationException {
		...
		return currentDevice.newInstance(currentDevice.getIdentity().getUdn(), currentDevice.getVersion(), currentDevice.getType(), currentDevice.getDetails(),
				iconDupes, currentDevice.toServiceArray(describedServices), describedEmbeddedDevices);
	}
