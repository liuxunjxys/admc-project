 ...
 * <p>
 * Writes the outgoing message's body with the {@link org.teleal.cling.transport.spi.SOAPActionProcessor}. This protocol will return <code>null</code> if no
 * response was received from the control target host. In all other cases, even if only the processing of message content failed, this protocol will return an
 * {@link org.teleal.cling.model.message.control.IncomingActionResponseMessage}. Any error details of a failed response (
 * {@link org.teleal.cling.model.message.UpnpResponse#isFailed()}) are available with
...
/*
 * 
 * - send request - UnsupportedDataException: Can't write body
 * 
 ...
 * 
 * - streamResponseMessage >= 300 && 405: Try request again with different headers - UnsupportedDataException: Can't write body - (The whole streamResponse
 * conditions apply again but this time, ignore 405)
 * 
 * - streamResponseMessage >= 300 && 500 && lastExecutionFailure != null: Try to read SOAP failure body - UnsupportedDataException: Can't read body
 * 
 * - streamResponseMessage < 300: Response was OK, try to read response body - UnsupportedDataException: Can't read body
 */