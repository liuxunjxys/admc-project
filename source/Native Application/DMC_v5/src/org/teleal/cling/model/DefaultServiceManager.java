 ...
 * <p>
 * Creates instance of the defined service class when it is first needed (acts as a factory), manages the instance in a field (it's shared), and synchronizes
 * (locks) all multi-threaded access. A locking attempt will timeout after 500 milliseconds with a runtime exception if another operation is already in
 * progress. Override {@link #getLockTimeoutMillis()} to customize this behavior, e.g. if your service bean is slow and requires more time for typical action
