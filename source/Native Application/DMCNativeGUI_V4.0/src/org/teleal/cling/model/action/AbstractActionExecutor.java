...
public abstract class AbstractActionExecutor implements ActionExecutor {
	...
	public void execute(final ActionInvocation<LocalService> actionInvocation) {
		...
		try {
			...
			log.log(Level.FINE, "Exception root cause: ", Exceptions.unwrap(ex));
			actionInvocation.setFailure(new ActionException(ErrorCode.ACTION_FAILED, "Action method invocation failed: "
					+ (ex.getMessage() != null ? ex.getMessage() : ex.toString()), ex));
		}
	}
	 ...
	 * 
	 * @param action
	 *            The action of which the output arguments are read.
	 * @param instance
	 *            The instance on which the accessors will be invoked.
	 * @return <code>null</code> if the action has no output arguments, a single instance if it has one, an <code>Object[]</code> otherwise.
	 * @throws Exception
	 */
	...
	protected void setOutputArgumentValue(ActionInvocation<LocalService> actionInvocation, ActionArgument<LocalService> argument, Object result)
		...
		if (result != null) {
			try {
				...
				throw new ActionException(ErrorCode.ARGUMENT_VALUE_INVALID, "Wrong type or invalid value for '" + argument.getName() + "': " + ex.getMessage(),
						ex);
			}
		} else {
