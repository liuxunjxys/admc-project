...
public abstract class SubscriptionCallback implements Runnable {
	...
	/**
	 * Called when establishing a local or remote subscription failed. To get a nice error message that transparently detects local or remote errors use
	 * <tt>createDefaultFailureMessage()</tt>.
	 * 
	 * @param subscription
	 *            The failed subscription object, not very useful at this point.
	 * @param responseStatus
	 *            For a remote subscription, if a response was received at all, this is it, otherwise <tt>null</tt>.
	 * @param exception
	 *            For a local subscription, any exception that caused the failure, otherwise <tt>null</tt>.
	 * @param defaultMsg
	 ...
	 * @param reason
	 *            If the subscription ended regularly (through <tt>end()</tt>), this is <tt>null</tt>.
	 * @param responseStatus
	 *            For a remote subscription, if the cause implies a remopte response and it was received, this is it (e.g. renewal failure response).
	 */
	 ...
	 * <p>
	 * Use the {@link org.teleal.cling.model.gena.GENASubscription#getCurrentValues()} method to obtain the evented state variable values.
	 * </p>
	 * 
	 * @param subscription
	 *            The established subscription with fresh state variable values.
	 */
	...
	/**
	 * @param responseStatus
	 *            The (HTTP) response or <code>null</code> if there was no response.
	 * @param exception
	 *            The exception or <code>null</code> if there was no exception.
