...
public class RouterImpl implements Router {
	...
	/**
	 * Creates a {@link org.teleal.cling.transport.spi.NetworkAddressFactory} from the given {@link org.teleal.cling.UpnpServiceConfiguration} and initializes
	 * listening services. First an instance of {@link org.teleal.cling.transport.spi.MulticastReceiver} is bound to eatch network interface. Then an instance
	 * of {@link org.teleal.cling.transport.spi.DatagramIO} and {@link org.teleal.cling.transport.spi.StreamServer} is bound to each bind address returned by
	 * the network address factory, respectively. There is only one instance of {@link org.teleal.cling.transport.spi.StreamClient} created and managed by this
	 * router.
	 * 
	 * @param configuration
	 ...
	 *             When initialization of any listening network service fails.
	 */
	...
	synchronized public List<NetworkAddress> getActiveStreamServers(InetAddress preferredAddress) {
		...
		if (preferredAddress != null && (preferredServer = getStreamServers().get(preferredAddress)) != null) {
			streamServerAddresses.add(new NetworkAddress(preferredAddress, preferredServer.getPort(), getNetworkAddressFactory().getHardwareAddress(
					preferredAddress)
	...
	/**
	 * Obtains the asynchronous protocol {@code Executor} and runs the protocol created by the {@link org.teleal.cling.protocol.ProtocolFactory} for the given
	 * message.
	 * <p>
	 * If the factory doesn't create a protocol, the message is dropped immediately without creating another thread or consuming further resoures. This means we
	 * can filter the datagrams in the protocol factory and e.g. completely disable discovery or only allow notification message from some known services we'd
	 * like to work with.
	 * </p>
	 * 
	 * @param msg
	...
	/**
	 * Obtains the synchronous protocol {@code Executor} and runs the {@link org.teleal.cling.transport.spi.UpnpStream} directly.
	 * 
	 * @param stream
	 *            The received {@link org.teleal.cling.transport.spi.UpnpStream}.
	 */
	 ...
	 * @return The return value of the {@link org.teleal.cling.transport.spi.StreamClient#sendRequest(StreamRequestMessage)} method or <code>null</code> if no
	 *         <code>StreamClient</code> is available.
	 */
	...
	/**
	 * Sends the given bytes as a broadcast on all bound {@link org.teleal.cling.transport.spi.DatagramIO}s, using source port 9.
	 * <p>
