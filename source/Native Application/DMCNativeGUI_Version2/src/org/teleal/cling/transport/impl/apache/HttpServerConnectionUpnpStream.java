...
public class HttpServerConnectionUpnpStream extends UpnpStream {
	...
	/**
	 * A thread-safe custom service implementation that creates a UPnP message from the request, then passes it to <tt>UpnpStream#process()</tt>, finally sends
	...
	protected class UpnpHttpService extends HttpService {
		...
		protected void doService(HttpRequest httpRequest, HttpResponse httpResponse, HttpContext ctx) throws HttpException, IOException {
			...
			if (responseMsg != null) {
				...
				// Status line
				httpResponse.setStatusLine(new BasicStatusLine(new ProtocolVersion("HTTP", 1, responseMsg.getOperation().getHttpMinorVersion()), responseMsg
						.getOperation().getStatusCode(), responseMsg.getOperation().getStatusMessage()));
