 ...
 * <p>
 * This service has no real network transport layer, it collects all messages instead and makes them available for testing with
 * {@link #getOutgoingDatagramMessages()}, {@link #getSentStreamRequestMessages()}, etc. Mock responses for TCP (HTTP) stream requests can be returned by
 * overriding {@link #getStreamResponseMessage(org.teleal.cling.model.message.StreamRequestMessage)} or {@link #getStreamResponseMessages()} if you know the
 * order of requests.
 * </p>
 * <p>
...
public class MockUpnpService implements UpnpService {
	 ...
	 * <p>
	 * The {@link org.teleal.cling.protocol.async.SendingNotificationAlive} protocol only sends messages if this feature is enabled when instantiating the
	 * factory.
	 * </p>
	 * <p>
	 * The {@link org.teleal.cling.protocol.async.SendingSearch} protocol doesn't wait between sending search message bulks, this speeds up testing.
	 * </p>
	 */
	...
	public class MockRouter implements Router {
		...
		public List<NetworkAddress> getActiveStreamServers(InetAddress preferredAddress) {
			...
			try {
				return Arrays.asList(new NetworkAddress(InetAddress.getByName("127.0.0.1"), NetworkAddressFactoryImpl.DEFAULT_TCP_HTTP_LISTEN_PORT));
		...
		public StreamResponseMessage send(StreamRequestMessage msg) {
			...
			counter++;
			return getStreamResponseMessages() != null ? getStreamResponseMessages()[counter] : getStreamResponseMessage(msg);
		}
