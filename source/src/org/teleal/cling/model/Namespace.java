 ...
 * <p>
 * Every descriptor, icon, event callback, or action message is send to a URL. This namespace defines how the path of this URL will look like and it will build
 * the path for a given resource.
 * </p>
 * <p>
 ...
 * </pre>
 * <p>
 * The namespace is also used to discover and create all {@link org.teleal.cling.model.resource.Resource}s given a {@link org.teleal.cling.model.meta.Device}'s
 * metadata. This procedure is typically invoked once, when the device is added to the {@link org.teleal.cling.registry.Registry}.
 * </p>
 * 
...
public class Namespace {
	...
	public URI getPath(Device device) {
		...
		/*
		 * We no longer need the hierarchical URIs, in fact, they are impossible to parse with typical URI template support in various REST engines. if
		 * (device.isRoot()) { s.append(device.getIdentity().getUdn().getIdentifierString()); } else { List<Device> devices = new ArrayList(); Device temp =
		 * device; while (temp != null) { devices.add(temp); temp = temp.getParentDevice(); } Collections.reverse(devices); for (Device d : devices) { if (d ==
		 * device) continue; s.append(d.getIdentity().getUdn().getIdentifierString()); s.append(EMBEDDED); s.append("/"); }
