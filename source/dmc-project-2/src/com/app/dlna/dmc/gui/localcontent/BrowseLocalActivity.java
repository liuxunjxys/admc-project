...
public class BrowseLocalActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (!checkSDCard()) {
			new AlertDialog.Builder(BrowseLocalActivity.this).setMessage("Sdcard is busy").setTitle("Error").setOnCancelListener(new OnCancelListener() {
			...
			for (File subFile : file.listFiles()) {
				if (subFile.isDirectory()) {
					...
					if (mimeType != null) {
						...
						res.add(new Res(new MimeType(mimeType.split("/")[0], mimeType.split("/")[1]), subFile.length(), Utility.createLink(subFile)));
